...
export async function getFeedAction({ take = 20 } = {}) {
  ...
  const chiefCourseIds = appUser.role === "TEACHER"
    ? await prisma.teacherCourse.findMany({
        where: { teacherId: appUser.id, isChief: true },
    ...
    where: {
      ...
      OR: [
        ...
        ...(appUser.courseId
          ...
          : []),
        ...(chiefCourseIds.length > 0
          ? [{ targets: { some: { type: "COURSE", courseId: { in: chiefCourseIds } } } }]
          : []),
      ],
    },
